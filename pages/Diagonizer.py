...
with open(path + "/../res/translation.json", encoding="utf-8") as f:
    transl = json.load(f)
...
# OpenAI API key
openai.api_key = "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"
...
if 'lang_select' in st.session_state:
    ...
    lang = st.sidebar.selectbox(transl[st.session_state['lang_tmp']]["language_selection"], options=list(transl.keys()),
                                key='lang_select')
...
logo_name = path + "/../images/Health Track.png"

# Define columns
t1, t2 = st.columns([1, 3], gap="large")
with t1:
    st.image(logo_name, caption='', width=None)
with t2:
